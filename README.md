...
- На этом этапе основные бизнес-данные хранятся в памяти API-процесса (без персистентной БД), а пользовательские сессии сохраняются в Redis.
...
- Таймаут graceful shutdown API настраивается через `GRACEFUL_SHUTDOWN_TIMEOUT_MS` (по умолчанию 10000 мс).
